...
class YumUpdateBlock extends IntervalBlock {
  ...
  update() {
    childProcess.exec('yum check-update | sed \'1,/^$/d\' | wc -l', (err, out) => {
      ...
      this._output = ` ${updates} `;

      // Accent if updates
      if (updates)
        this._output = `%{B${config.get('cAccent')}} ${
          this._output
        } %{B${config.get('cBG')}}`

      this._output = util.addAction(this._output,
  ...
  update() {
    this.emitter.workspaces((err, data) => {
      ...
      this._output = data.map(function(space) {
        ...
        current = util.addAction(` ${current} `,
            `i3-msg workspace "${space.name}"`);
        if (space.urgent)
          current =
            `%{B${config.get('cUrgent')}}${current}%{B${config.get('cBG')}}`;
        if (space.focused)
          current =
            `%{B${config.get('cAccent')}}${current}%{B${config.get('cBG')}}`;
  ...
  update() {
    this.emitter.tree((err, data) => {
      ...
      this._output = ` ${_.trunc(util.findFocused(data).name, {
        length:     config.get('titleMaxlen'),
        omission:   config.get('titleTrunc')
      })} `;
    ...
    this.fudge = setTimeout(() => {
      linuxBattery().then((batteries) => {
        ...
        if (percent <= config.get('batCritical'))
          this._output = `%{B${config.get('cUrgent')}} ${
            this._output
          } %{B${config.get('cBG')}}`
  ...
  update() {
    ...
    this.fudge = setTimeout(() => {
      childProcess.exec('amixer get Master', (err, out) => {
        ...
        let state = matches[1];
        if (matches[2] === 'off')
          state = `%{B${config.get('cUrgent')}} ${state} %{B${config.get('cBG')}}`;
        else
          state = ` ${state} `;

        // Commands
        // Toggle mute
        state = util.addAction(state,
            'amixer set Master toggle', 1, true);
        // Volume up
        state = util.addAction(state,
