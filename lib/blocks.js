...
class AnimatedExpandableBlock extends ExpandableBlock {
  // Add expanding state
  constructor(blocks, minText, maxText, buttonOnRight, startsExpanded) {
    super(blocks, minText, maxText, buttonOnRight, startsExpanded);
  ...
  update() {
    ...
    blocks = this.buttonOnRight ? `${blocks} ` : ` ${blocks}`;
    ...
    if (this.startExpand) {
      ...
      if (this.expanded) {
        this.interval = setInterval(() => {
          this.index = util.nextAnimationIdx(animArr, this.index, false);
          if (this.index == animArr.length - 1) {
            clearInterval(this.interval);
            this.expanding = false;
          }
          this.update();
        }, config.get('animFrameLen'));
      } else {
        this.interval = setInterval(() => {
          this.index = util.nextAnimationIdx(animArr, this.index, true);
          if (this.index == 0) {
            clearInterval(this.interval);
            this.expanding = false;
          }
    ...
    if (this.buttonOnRight)
      animBlocks = animArr.slice(0, this.index).join('');
    else
      animBlocks = animArr.slice(animArr.length - this.index, animArr.length).join('');
  ...
  update() {
    this.emitter.workspaces((err, data) => {
      ...
      this._output = data.map(function(space) {
        ...
        // Prev workspace
        current = util.addAction(current, "i3-msg worskpace prev", 5);
