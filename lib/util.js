...
function nextAnimationIdx(animArr, idx, backwards) {
  let skip;
  if (!backwards) {
    skip = idx < animArr.length - 1 && animArr[idx + 1].match(CONTROL_BLOCK_REGEX);
    return Math.min(animArr.length - 1, idx + (skip ? 2 : 1));
  } else {
    skip = idx > 1 && animArr[idx - 1].match(CONTROL_BLOCK_REGEX);
    return Math.max(0, idx - (skip ? 2 : 1));
  }
}
